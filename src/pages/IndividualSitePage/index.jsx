...
const IndividualSitePage = () => {
  ...
  const { data, error } = useGetDoc("Places", place_id)

  if (error) {
    return <ErrorPage />
  }

  if (!data) return <SplashScreen allTheWayUp={false} />

  if (data)
    return (
      <>
        <GridViewForDekstops>
          <Carousel images={data.otherimages} />
        ...
        >
          <Tabs.Tab label="Details" icon={<Document />}>
            <DetailsTab
              city={data.cityname}
              address={data.address}
              coords={data.geopoints}
            />
          </Tabs.Tab>

          <Tabs.Tab label="Comments" icon={<Chat />}>
            <CommentsTab place_id={place_id} />
          </Tabs.Tab>
        </Tabs>
      </>
    )
