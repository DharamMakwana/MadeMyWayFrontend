...
const ActionButtons = ({ place_id }) => {
  ...
  return (
    <ButtonContainer>
      <SaveActionButton place_id={place_id} />

      <Button onClick={() => navigate(`/site/${place_id}`)} size="sm">
        Show Details
      </Button>
    </ButtonContainer>
  )
...
export function SiteCard({ info }) {
  return (
    <CardFigure style={{ position: "relative" }}>
      <Image src={info.thumbnail} height={190} />

      <Badge
        variant="filled"
        color={info.status ? "green" : "gray"}
        style={{ position: "absolute", left: "14px", top: "16px" }}
      >
        {info.status ? "Verified" : "Under Process"}
      </Badge>

      <SiteDetails>
        <SiteLocation cityname={info.cityname} placename={info.placename} />
        <SitePraiseCounter
      ...
      <ActionButtons place_id={info.place_id} />
    </CardFigure>
  )
}
